  ...
  <div style="background-color:white;">
    <v-container grid-list-sm style="padding-top:5px;">
      <v-layout row wrap>
        <v-flex xs2>
          <v-card
            v-for="(catalogType, i) in this.listCatalogType"
            :key="i"
            elevation="5"
          ...
          >
            ...
            >{{ catalogType.type_name }}</v-card-title
            >
            <v-divider />
            <v-list style="padding-top:0px" dense>
              <v-list-tile-content>
                <v-list-tile
                  v-for="(catalogs, k) in catalogType.catalogs"
                  :key="k"
          ...
          <v-card elevation="5" style="margin-top:5px">
            <v-card-title
              color="teal"
              class="font-weight-medium"
              style="padding-bottom:15px"
            >Nhà cung cấp</v-card-title
            >
            <v-divider />
            <v-list style="padding-top:0px" dense>
              <v-list-tile-content>
                <v-list-tile
                  v-for="(supplier, i) in this.suppliers"
                  :key="i"
                  @click="choseCategory(detail)"
                >
                  <v-list-tile-sub-title class="black--text body-1 pl-3">{{
        ...
        <v-flex xs10>
          <v-layout row wrap>
            <v-flex
              v-for="(product, i) in this.currentProduct"
              :key="i"
              xs6
              md3
            >
...
export default {
  ...
  watch: {
    $route: {
      immediate: true,
      async handler($route) {
        this.response = $route.params.item;
        const { data, success, message } = await this.listProductDetail({
          typeDetail: $route.params.item
        });
        if (data.length === 0) {
        } else {
  ...
  methods: {
    ...
    async suggestItem() {
      const { data, success, message } = await this.suggestListItem({
        cateDetailsType: this.response
      });

      if (data.length > 0) {
        let result = data[0];
        this.listCatalogType = result.catalog_types;
      } else {
        this.listCatalogType = [];
      }
    },
    ...
    async listSupplier() {
      const { data, success, message } = await this.suggestSupplier({
        cateDetailsType: this.response
      });
      if (data.length > 0) {
        const result = data[0];
        this.suppliers = result.supplier_profiles;
        console.log(data);
      } else {
        this.suppliers = [];
      }
